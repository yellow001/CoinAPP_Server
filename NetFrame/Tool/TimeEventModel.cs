...
namespace NetFrame.Tool
{
    public class TimeEventModel
    {
        ...
        public TimeEventModel(long time, int count, Action de) {
            ...
            this.Wait_time = time* Util.Second_Ticks;//把秒数转换成 ticks 的单位长度
        ...
        public TimeEventModel(float time, int count, Action de) {
            ...
            this.Wait_time = (long)(time * Util.Second_Ticks);//把秒数转换成 ticks 的单位长度
            this.Excute_time = DateTime.Now.Ticks / Util.Second_Ticks + Wait_time;
            this.count = count;
            this.de = de;
        }
